...
2. 高德转为**核实层**：POI 搜索拿坐标、补距离与营业状态（原为地理编码，见 §8 修订）
3. 偏好采集表单 + 显式「帮我推荐」触发
...
skill 的来源是**网页检索核实**，不是凭记忆生成。所以高德查不到，更可能是
「高德没收录这个小店」，而非「AI 编造的」。

丢弃它等于**拿我们核实能力的短板去惩罚用户**，并浪费 skill 的检索成果。
...
> **2026-09-19 修订：坐标来源从地理编码改为 POI 关键字搜索。**
> 原文写的是「`/v3/geocode/geo`，用 `address` 查询」。地理编码回答的是
> 「这个地址在哪」，而这里手上是一个 **POI 名字**。拿名字去地理编码，高德做的是
> 字符串匹配而不是找 POI：实测把「厦门老院子景区」放到了厦门北站的麦当劳、
> 「厦门市图书馆集美新城馆区」放到了一公里外的住宅楼、「海堤路（集美段）」
> 放到了集美学村里的塘埔路（离真正的海堤路 1.5 公里）。同一个名字走关键字搜索，
> 8/8 全部命中。上面那条「显示错位」的用户反馈就是这么来的。
>
> 也**不要**改回「把 `name` 拼进 `address` 再地理编码」：实测名字越完整越准
> （「海堤路（集美段）」能搜到集美区那条，「海堤路」反而落到湖里区），
> 所以问题出在用错了接口，不是查询词不够长。同理，**不要**改成
> 「在候选里挑离出发点最近的」—— 真正的沙坡尾在思明区、离出发点 14.5 公里，
> 而集美大悦城有个同名的「沙坡尾酒场」在一公里内，按距离挑会把对的换成错的。
> 高德的相关度排序实测稳定（连查三次逐位相同），直接取 `pois[0]` 即可。

对 skill 返回的每条 `recommendations[]`：

1. **POI 关键字搜索**：`/v3/place/text`，用 `name`（含城市）查询，取 `pois[0]`
   - 命中 → 得到坐标与高德 POI 名称，`verified: true`
   - 未命中 → `point: null`，`verified: false`，保留在列表但不参与地图与路线。
     **不退而求其次用地理编码** —— 一个自信的错点比一个诚实的「未核实」更糟：
     错点会画上图、算错距离，而「未核实」由界面如实标注、不占编号、不参与路线
2. **距离**：用搜索得到的坐标与出发点算**直线距离**（`haversineMeters`）。
   注意这与第一版的「驾车距离」不是一回事，界面上必须标明是直线距离，
   不能让用户以为是实际行车里程。真实行车距离只在规划出路线之后才有
3. **营业状态**：`biz_ext` 就在同一条搜索响应里，直接取匹配到的那个 POI 自己的。
   不必再打一次周边搜索 —— 那不但多一次串行请求（核实是首屏瓶颈），
   而且取的是「坐标 200 米内最近那个 POI」的营业时间，未必是这个地点的。
   解析失败或查不到一律给 `unknown`。**绝不允许把「无法判断」当成「已关闭」**

核实走既有的 `amap-fetch`，**复用 QPS 退避重试**（见前置文档 9.1）。
...
- POI 搜索结果映射：命中与未命中两种分支

**不测**：DeepSeek 真实调用、高德真实 POI 搜索（靠手工验收）。
