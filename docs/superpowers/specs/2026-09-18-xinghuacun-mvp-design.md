...
- 日期：2026-09-18
- 状态：**部分被取代** —— 见 `2026-09-19-recommend-layer-design.md`

> **取代说明（2026-09-19）**：用户实测后确认高德 `place/around` 的召回质量不可用
> （返回寄存处、售票处、写字楼，根因是未传 `types` 导致高德返回默认的三类），
> 决定改为 AI（DeepSeek + 用户自备 skill）出推荐、高德退为核实层。
>
> 因此本文件中**第 2 节（本版范围）、第 7 节（排序规则）、第 8 节（游玩项目推导）
> 已被取代**，对应的 `rank.ts` / `derive.ts` / `providers/poi/*` 将删除。
> 其余各节（密钥模型、目录结构、API 契约、降级、QPS 约束、设计方向）继续有效。
